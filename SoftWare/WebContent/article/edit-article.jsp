...
<section class="container-fluid">
      ...
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
          <ul class="nav navbar-nav navbar-right">
            ...
            <li><a href="login/loginOut" onClick="if(!confirm('真要回家吃饭了嘛，哼(⊙_⊙)?'))return false;">回家吃饭</a></li>
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <form action="article/edit" method="post" class="add-article-form">
          <div class="col-md-9">
            <h1 class="page-header">修改Blog</h1>
            <div class="form-group">
              <label for="article-title" class="sr-only">标题</label>
              <input type="text" id="article-title" name="blog.btitle" class="form-control" placeholder="在此处输入标题" required autofocus autocomplete="off" value="${blog.btitle }">
            </div>
            <div class="form-group">
              <label for="article-content" class="sr-only">内容</label>
              <script id="article-content" name="blog.bcontent" type="text/plain" >${blog.bcontent }</script>
            </div>
           
            <div class="add-article-box">
              <h2 class="add-article-box-title"><span>描述</span></h2>
              <div class="add-article-box-content">
              	<textarea class="form-control" name="blog.bdes" autocomplete="off" >${blog.bdes }</textarea>
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
                <ul class="category-list">
                  <li>
                    <label>
                      <input name="blog.btype" type="radio" value="情感类" <c:if test="${blog.btype == '情感类' }">checked="checked"</c:if> >
                      情感类<em class="hidden-md">(  <span>1</span> )</em></label>
                  </li>
                  <li>
                    <label>
                      <input name="blog.btype" type="radio" value="生活类" <c:if test="${blog.btype == '生活类' }">checked="checked"</c:if> >
                      生活类 <em class="hidden-md">(  <span>2</span> )</em></label>
                  </li>
                  <li>
                    <label>
                      <input name="blog.btype" type="radio" value="发牢骚" <c:if test="${blog.btype == '发牢骚' }">checked="checked"</c:if> >
                     没事 发发牢骚 <em class="hidden-md">(  <span>3</span> )</em></label>
                  </li>
                  <li>
                    <label>
                      <input name="blog.btype" type="radio" value="学习类" <c:if test="${blog.btype == '学习类' }">checked="checked"</c:if> >
                      我要学习 <em class="hidden-md">( <span>4</span> )</em></label>
                  </li>
                  <li>
                    <label>
                      <input name="blog.btype" type="radio" value="毒鸡汤" <c:if test="${blog.btype == '毒鸡汤' }">checked="checked"</c:if> >
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
              	<p><label>状态：</label><span class="article-status-display">已发布</span></p>
               	<p><label>公开度：</label><input type="radio" id="public" name="blog.encrypt" value="0" onclick="to_change()" <c:if test="${blog.encrypt =='0' }">checked="checked"</c:if> />公开 <input type="radio" id="private" name="blog.encrypt" value="1" onclick="to_change()" <c:if test="${blog.encrypt =='1' }">checked="checked"</c:if> />加密</p>
                ...
                <p><label>发布于：</label><span class="article-time-display"><input style="border: none;" type="datetime" name="blog.btime" value=<%=time %> /><input type="hidden" name="blog.snumber" value="${student.snumber }"/><input type="hidden" name="blog.bId" value="${blog.bId }"/></span></p>
              </div>
